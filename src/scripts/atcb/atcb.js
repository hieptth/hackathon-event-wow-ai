...
function atcb_init() {
  ...
  if (atcButtons.length > 0) {
    ...
    const atcButtonsInitialized =
      document.querySelectorAll(".atcb-initialized");
      ...
      if (atcb_check_required(atcbConfig)) {
        ...
        if (atcb_validate(atcbConfig)) {
          ...
          if (atcbConfig.identifier == null || atcbConfig.identifier == "") {
            atcbConfig.identifier =
              "atcb-btn-" + (i + atcButtonsInitialized.length + 1);
          }
          // generate the button
...
function atcb_patch_config(atcbConfig) {
  ...
  Object.keys(keyChanges).forEach((key) => {
    if (
      atcbConfig[keyChanges[`${key}`]] == null &&
      atcbConfig[`${key}`] != null
    ) {
...
function atcb_decorate_data(atcbConfig) {
  ...
  for (let i = 0; i < atcbConfig.options.length; i++) {
    ...
    atcbConfig.options[`${i}`] = cleanOption[0]
      .toLowerCase()
  ...
  if (!atcbConfig.description || atcbConfig.descriptionHtmlFree)
    return atcbConfig;
...
function atcb_generate_label(
  ...
  let defaultTriggerText = atcb_translate_hook(
    "Add to Calendar",
    data.language,
    data
  );
...
function atcb_generate(button, data) {
  ...
  if (data.name && data.location && data.startDate) {
    ...
    if (data.descriptionHtmlFree)
      schemaEl.textContent +=
        '"description":"' + data.descriptionHtmlFree + '", ';
    const formattedDate = atcb_generate_time(
      data,
      "delimiters",
      "general",
      true
...
function atcb_open(
  ...
  const bgOverlay = atcb_generate_bg_overlay(
    data.listStyle,
    data.trigger,
    data.background
  );
...
export function atcb_action(data, triggerElement, keyboardTrigger = true) {
...
function atcb_generate_teams(data) {
  ...
  if (data.descriptionHtmlFree != null && data.descriptionHtmlFree != "") {
    ...
    url +=
      "&content=" +
      locationString +
      encodeURIComponent(data.descriptionHtmlFree);
  }
  if (atcb_secure_url(url)) {
    // eslint-disable-next-line security/detect-non-literal-fs-filename
    window.open(url, atcbDefaultTarget).focus();
  ...
  if (data.startTime != null && data.endTime != null) {
    ...
    if (addTimeZoneOffset) {
      ...
      if (data.timeZoneOffset != null && data.timeZoneOffset != "") {
        ...
        let tzOffsetDateStart = new Date(
          start.toLocaleString("sv", { timeZone: data.timeZone })
        );
        let tzOffsetStartSearch = tzOffsetDateStart
          .toString()
          .match(/GMT(.{5})/g);
        tzOffsetStart = tzOffsetStartSearch[0].replace(
          /GMT(.{3})(.{2})/g,
          "$1:$2"
        );
        let tzOffsetDateEnd = new Date(
          end.toLocaleString("sv", { timeZone: data.timeZone })
        );
  ...
  if (anchorSet === true && !list.classList.contains("atcb-dropoverlay")) {
    ...
    if (
      ...
      list.style.bottom =
        2 * viewportHeight -
        (viewportHeight +
          (btnDim.top + (btnDim.top + btnDim.height - triggerDim.top))) -
        window.scrollY +
        "px";
    } else {
    ...
    list.style.top =
      ...
      "px";
    list.style.left =
      triggerDim.left - (listDim.width - triggerDim.width) / 2 + "px";
  }
}
  ...
  document.addEventListener("keydown", (event) => {
    if (
      ...
      const optionListCount =
        document.querySelectorAll(".atcb-list-item").length;
      if (currFocusOption.classList.contains("atcb-list-item")) {
        if (
          event.key === "ArrowDown" &&
          currFocusOption.dataset.optionNumber < optionListCount
        ) {
          event.preventDefault();
          targetFocus = parseInt(currFocusOption.dataset.optionNumber) + 1;
        } else if (
          event.key === "ArrowUp" &&
          currFocusOption.dataset.optionNumber >= 1
        ) {
          event.preventDefault();
          targetFocus = parseInt(currFocusOption.dataset.optionNumber) - 1;
        }
        if (targetFocus > 0) {
          document
            .querySelector(
              '.atcb-list-item[data-option-number="' + targetFocus + '"]'
            )
            .focus();
        }
      } else {
        ...
        if (
          ...
          document
            .querySelector(
              '.atcb-list-item[data-option-number="' + optionListCount + '"]'
            )
            .focus();
        } else {
          document
